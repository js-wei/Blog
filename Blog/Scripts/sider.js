...
　　　　};
	  $this=$(this);
	  $('.banner-image>li').first().css('z-index',1);
　　  var opts = $.extend(defaults,options);
	  ...
	  if(opts.point){
		$this.append($html);
		$('.banner-list>ul').css('z-index',2).width($length*50);	//设置生成的点
	  }
	  ...
	  $('.banner-list li').on('mouseover',function() {
			...
			$('.banner-list>ul').css('z-index',2);
			$show.css('z-index',1).siblings().removeAttr('style');
	  });
	  if(opts.auto){
